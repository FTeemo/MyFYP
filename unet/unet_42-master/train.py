...
def train_net(net, device, data_path, epochs=20, batch_size=5, lr=0.00001):
    # 加载训练集
    # isbi_dataset = ISBI_Loader(data_path)
    #imgs_path = "D:/dataset/Test_Images"
    isbi_dataset = ISBI_Loader(data_path)
    per_epoch_num = len(isbi_dataset) / batch_size
    train_loader = torch.utils.data.DataLoader(dataset=isbi_dataset, batch_size=batch_size, shuffle=False)
    # 定义RMSprop算法
...
if __name__ == "__main__":
    ...
    # 指定训练集地址，开始训练
    data_path = "/root/MyFYP/Dataset_new/dataset/"  # todo 修改为你本地的数据集位置
